...
export function UploadTool({ type, refresh }) {
  ...
  const submit = async e => {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        await upload(endpoint, form, value => setProgress(Math.round((i + value / 100) / batches.length * 100)))
        accepted++; refresh()
...
export function YouTubeTool({ refresh }) {
  ...
  const submit = async e => {
    ...
    try {
      await request('/api/youtube/download', { method: 'POST', body: { url: url.trim(), outputType: format, quality, audioQuality: audio } })
  ...
  return <form className="tool-form" onSubmit={submit}>
    ...
    </div><button className="primary" disabled={!url.trim() || busy}>{busy ? 'Adding…' : `Download ${format.toUpperCase()}`}</button>
    <Notice>{message}</Notice><Notice error>{error}</Notice>
  </form>
}
export function LinkTool({ refresh }) {
  ...
  const submit = async e => {
    ...
    for (const url of urls) {
      try { await request('/api/remote/download', { method: 'POST', body: { url } }); count++; refresh() }
